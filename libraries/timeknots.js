var TimeKnots = {
    draw: function(id, events, options) {
        ...
        svg.selectAll("line")
            ...
            .attr("x1", function(d) {
                ...
                if (cfg.horizontalLayout) {
                    ...
                    ret = Math.max(Math.floor(cfg.width / 2) - TIMELINE_LEFT_MARGIN, cfg.radius + 5);
                }
                linePrevious.x1 = ret
                return ret
            })
            .attr("x2", function(d) {
                ...
                return Math.max(Math.floor(cfg.width / 2) - TIMELINE_LEFT_MARGIN, cfg.radius +5 );
            })
            .attr("y1", function(d) {
                var ret;
                if (cfg.horizontalLayout) {
        ...
        svg.selectAll("text")
            ...
            .attr("x", function(d) {
                return Math.max(Math.floor(cfg.width / 2) - TEXT_LEFT_MARGIN, cfg.radius * 3 +5);
            })
            .attr("y", function(d) {
                var datum = (cfg.dateDimension) ? new Date(d.date).getTime() : d.value;
